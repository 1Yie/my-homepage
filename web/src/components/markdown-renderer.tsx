import type { Element as HastElement, Text as HastText, Root } from 'hast';
...
export function MarkdownRenderer({
	...
	return (
		<div
			className={`prose prose-sm max-w-none dark:prose-invert prose-pre:my-0
				post-style ${className || ''}`}
						...
						children: [
							...
							},
							{
								...
								properties: {
									class: 'code-copy',
									title: '复制代码',
									onclick: `event.preventDefault(); navigator.clipboard.writeText(this.parentElement.parentElement.querySelector('code').textContent).then(() => { if (window.toast) window.toast.success('内容已复制到剪贴板'); }).catch(() => { if (window.toast) window.toast.error('复制失败'); });`,
								},
								children: [
									{
										type: 'element',

...
function Toolbar({ className, ...props }: ToolbarPrimitive.Root.Props) {
	return (
		<ToolbarPrimitive.Root
			className={cn(
				`relative flex gap-2 rounded-xl border bg-card not-dark:bg-clip-padding
				p-1 text-card-foreground`,
				className
			)}
			data-slot="toolbar"
			{...props}
...
function ToolbarSeparator({
	...
	return (
		<ToolbarPrimitive.Separator
			className={cn(
				`shrink-0 bg-border data-[orientation=horizontal]:my-0.5
				data-[orientation=vertical]:my-1.5 data-[orientation=horizontal]:h-px

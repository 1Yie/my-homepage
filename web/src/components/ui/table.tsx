...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
	return (
		...
		>
			<table
				className={cn(
					`w-full caption-bottom in-data-[slot=frame]:border-separate
					in-data-[slot=frame]:border-spacing-0 text-sm`,
					className
				)}
				data-slot="table"
				{...props}
...
function TableHeader({ className, ...props }: React.ComponentProps<'thead'>) {
	return (
		<thead
			className={cn(
				`[&_tr]:border-b in-data-[slot=frame]:**:[th]:h-9
...
function TableBody({ className, ...props }: React.ComponentProps<'tbody'>) {
	return (
		<tbody
			className={cn(
				`relative in-data-[slot=frame]:rounded-xl
				in-data-[slot=frame]:shadow-xs/5 before:pointer-events-none
				before:absolute before:inset-px not-in-data-[slot=frame]:before:hidden
				...
				[&_tr:last-child]:border-0 in-data-[slot=frame]:*:[tr]:border-0
...
function TableFooter({ className, ...props }: React.ComponentProps<'tfoot'>) {
	return (
		<tfoot
			className={cn(
				`border-t in-data-[slot=frame]:border-none bg-muted/72
				in-data-[slot=frame]:bg-transparent font-medium [&>tr]:last:border-b-0
				in-data-[slot=frame]:*:[tr]:hover:bg-transparent`,
				className
			)}
...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
	return (
		<tr
			className={cn(
				`border-b transition-colors hover:bg-muted/72
...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
	return (
		<th
			className={cn(
				`h-10 whitespace-nowrap px-2.5 text-left align-middle font-medium
				text-muted-foreground leading-none has-[[role=checkbox]]:w-px

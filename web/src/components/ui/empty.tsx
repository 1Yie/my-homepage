				...
				<>
					<div
						...
						className={cn(
							...
							`-translate-x-0.5 -rotate-10 pointer-events-none absolute
							bottom-px origin-bottom-left scale-84 shadow-none`
						)}
					/>
					<div
						aria-hidden="true"
						className={cn(
							...
							translate-x-0.5 rotate-10 scale-84 shadow-none`
						)}
					/>
				</>
			)}
...
function EmptyDescription({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<div
			className={cn(
				`text-muted-foreground text-sm [&>a:hover]:text-primary [&>a]:underline
				[&>a]:underline-offset-4 [[data-slot=empty-title]+&]:mt-1`,
				className
			)}
...
function EmptyContent({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			className={cn(
				`flex w-full min-w-0 max-w-sm flex-col items-center gap-4 text-balance
				text-sm`,
				className
			)}

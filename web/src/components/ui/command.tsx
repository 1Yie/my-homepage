...
function CommandDialogBackdrop({
	...
	return (
		<CommandDialogPrimitive.Backdrop
			className={cn(
				`fixed inset-0 z-50 bg-black/32 backdrop-blur-sm transition-all
...
function CommandDialogViewport({
	...
	return (
		<CommandDialogPrimitive.Viewport
			className={cn(
				`fixed inset-0 z-50 flex flex-col items-center px-4
				py-[max(--spacing(4),4vh)] sm:py-[10vh]`,
				className
			)}
		...
		<CommandDialogPortal>
			...
			<CommandDialogViewport>
				<CommandDialogPrimitive.Popup
					className={cn(
						`-translate-y-[calc(1.25rem*var(--nested-dialogs))] relative
						row-start-2 flex max-h-105 min-h-0 w-full min-w-0 max-w-xl
						scale-[calc(1-0.1*var(--nested-dialogs))] flex-col rounded-2xl
						border bg-popover not-dark:bg-clip-padding text-popover-foreground
						opacity-[calc(1-0.1*var(--nested-dialogs))] shadow-lg/5 outline-none
						transition-[scale,opacity,translate] duration-200 ease-in-out
						will-change-transform before:pointer-events-none before:absolute
						before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)]
						before:bg-muted/72 before:shadow-[0_1px_--theme(--color-black/6%)]
...
function CommandPanel({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			className="-mx-px not-has-[+[data-slot=command-footer]]:-mb-px relative
				min-h-0 rounded-t-xl not-has-[+[data-slot=command-footer]]:rounded-b-2xl
				border border-b-0 bg-popover not-dark:bg-clip-padding shadow-xs/5
	...
	return (
		<div
			className={cn(
				...
				rounded-b-[calc(var(--radius-2xl)-1px)] border-t px-5 py-3

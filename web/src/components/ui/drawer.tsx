	...
	return (
		<DrawerPrimitive.Backdrop
			className={cn(
				...
				data-swiping:duration-0 supports-[-webkit-touch-callout:none]:absolute`,
				className
			)}
			...
			<DrawerViewport position={position} variant={variant}>
				<DrawerPrimitive.Popup
					className={cn(
						...
						will-change-transform [--peek:calc(--spacing(6)-1px)]
						...
						[--stack-step:0.05] before:pointer-events-none before:absolute
						before:inset-0 before:shadow-[0_1px_--theme(--color-black/4%)]
						after:pointer-events-none after:absolute after:bg-popover
						data-swiping:select-none data-nested-drawer-open:overflow-hidden
						...
						position === 'bottom' &&
							...
							has-data-[slot=drawer-bar]:pt-2 data-ending-style:mb-0
							data-starting-style:mb-0 data-ending-style:pb-0
						...
						position === 'left' &&
							...
							w-[calc(100%-(--spacing(12)))] max-w-md border-e after:inset-y-0
						...
						variant === 'inset' &&
							`before:hidden sm:rounded-2xl sm:border sm:after:bg-transparent
...
export function DrawerMenuTrigger({
	...
	return (
		<DrawerTrigger
			className={cn(
				`flex min-h-9 w-full cursor-default select-none items-center gap-2
				rounded-sm px-2 py-1 text-base text-foreground outline-none
				hover:bg-accent hover:text-accent-foreground sm:min-h-8 sm:text-sm
...
export function DrawerMenuCheckboxItem({
	...
	return (
		<CheckboxPrimitive.Root
			checked={checked}
			className={cn(
				`grid min-h-9 w-full cursor-default select-none items-center gap-2
				rounded-sm px-2 py-1 text-base text-foreground outline-none
				hover:bg-accent hover:text-accent-foreground
				data-disabled:pointer-events-none data-disabled:opacity-64 sm:min-h-8
				sm:text-sm [&_svg:not([class*='opacity-'])]:opacity-80
				[&_svg:not([class*='size-'])]:size-4.5
				sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none
			...
			{variant === 'switch' ? (
				<>
					...
					<CheckboxPrimitive.Indicator
						className="inset-shadow-[0_1px_--theme(--color-black/4%)]
